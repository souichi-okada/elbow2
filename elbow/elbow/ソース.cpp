...
int main() {
	...
	fp1 = fopen("nishihara_mae2.csv", "r");
	...
	for (j = 0;j <data;j++) {
		...
		//printf("%g\n", time[j]);
			...
			for (i = 31;i < 32;i++) {
				fscanf(fp1, "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%s", &joint_x[j][i], &joint_y[j][i], &joint_z[j][i], &data1[j][i], &data2[j][i], &data3[j][i], &data4[j][i], c[j][i]);
					//printf("%lf,%lf,%lf,%lf,%lf,%lf,%lf,%s\n", joint_x[j][i], joint_y[j][i], joint_z[j][i], data1[j][i], data2[j][i], data3[j][i], data4[j][i], c[j][i]);
	...
	for (i = 0;i < data;i++) {
		...
		knee_right[i][0] = joint_x[i][23] - joint_x[i][22];//右膝
	   knee_right[i][1] = joint_y[i][23] - joint_y[i][22];
		knee_right[i][2] = joint_z[i][23] - joint_z[i][22];
		...
		toe_right[i][0] = joint_x[i][24] - joint_x[i][23];//右足首
		toe_right[i][1] = joint_y[i][24] - joint_y[i][23];
		toe_right[i][2] = joint_z[i][24] - joint_z[i][23];
	...
	for (i = 0;i < data;i++) {
		...
		razian[i][2] = acos((neck[i][0] * toe_right[i][0] + neck[i][1] * toe_right[i][1] + neck[i][2] * toe_right[i][2]) / (sqrt(neck[i][0] * neck[i][0] + neck[i][1] * neck[i][1] + neck[i][2] * neck[i][2]) * sqrt(toe_right[i][0] * toe_right[i][0] + toe_right[i][1] * toe_right[i][1] + toe_right[i][2] * toe_right[i][2])));
		degree[i][2] = razian[i][2] * 180.0 / M_PI;
		razian[i][1] = acos((neck[i][0] * knee_right[i][0] + neck[i][1] * knee_right[i][1] + neck[i][2] * knee_right[i][2]) / (sqrt(neck[i][0] * neck[i][0] + neck[i][1] * neck[i][1] + neck[i][2] * neck[i][2]) * sqrt(knee_right[i][0] * knee_right[i][0] + knee_right[i][1] * knee_right[i][1] + knee_right[i][2] * knee_right[i][2])));
